-- ============================================================================
-- 0008_candidates_page_order_by_score.sql
--   Order search_candidates_page() by each candidate's BEST cached match_score
--   (desc, nulls last), then name. Uses the applications.match_score cache so
--   the list sorts by match across ALL pages — not just within one page.
-- ============================================================================
...
as $$
  ...
  select f.id, count(*) over () as total
   ...
   order by
     coalesce(
       (select max(a.match_score) from public.applications a where a.candidate_id = f.id),
       -1
     ) desc,
     f.full_name asc
